<template>
    <div id="app">
        ...
        <Footer :userCount="Object.keys(this.users).length" :postCount="Object.keys(this.posts).length"/>
    </div>
</template>
    ...
    beforeCreate() {
      this.$root.$on("onRegister", (login, username) => {
        ...
        if (!login || login.length < 3 | login.length > 16) {
          this.$root.$emit("onRegisterValidationError", "Login should be between 3 and 16 symbols");
        } else if (!/^[a-z]*$/.test(login)) {
          this.$root.$emit("onRegisterValidationError", "Login can contain only small latin letters");
          ...
          this.$root.$emit("onRegisterValidationError", "Name should be between 1 and 32 symbols");
        } else {
