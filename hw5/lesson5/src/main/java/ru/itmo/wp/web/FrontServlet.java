...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        if (isNull(lang) || !lang.matches("^[a-z]{2}$")) {
        ...
        Map<Class<?>, Object> fieldGetter = new HashMap<>();
        fieldGetter.put(HttpServletRequest.class, request);
        fieldGetter.put(Map.class, view);
        Object[] methodArguments = new Object[method.getParameterCount()];
        int countMethods = 0;
        for (Class<?> parameter : method.getParameterTypes()) {
            methodArguments[countMethods++] = fieldGetter.get(parameter);
        }

        method.setAccessible(true);
        try {
            method.invoke(page, methodArguments);
        ...
        StringBuilder templateName = new StringBuilder(pageClass.getSimpleName());
        if (!isNull(lang)) {
            templateName.append("_");
            templateName.append(lang);
            templateName.append(".ftlh");
        }
        Template template = newTemplate(templateName.toString());
        if (isNull(template)) {
