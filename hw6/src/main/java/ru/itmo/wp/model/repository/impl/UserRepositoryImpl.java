...
public class UserRepositoryImpl implements UserRepository {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT COUNT(*) FROM User")) {
                try (ResultSet resultSet = statement.executeQuery()) {
                    if (!resultSet.next()) {
                        throw new RepositoryException("Failure when counting users");
                    }
                    return resultSet.getInt(1);
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't access database.", e);
        }
    }
    @Override
    ...
    private User findBy(String field, String value) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    String.format("SELECT * FROM User WHERE %s=?", field))) {
    ...
    private User findByFieldAndPasswordSha(String field, String value, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    String.format("SELECT * FROM User WHERE %s=? AND passwordSha=?", field))) {
                statement.setString(1, value);
    ...
    public User findByLoginAndPasswordSha(String login, String passwordSha) {
        return findByFieldAndPasswordSha("login", login, passwordSha);
    }

    @Override
    public User findByEmailAndPasswordSha(String email, String passwordSha) {
        return findByFieldAndPasswordSha("email", email, passwordSha);
    }
