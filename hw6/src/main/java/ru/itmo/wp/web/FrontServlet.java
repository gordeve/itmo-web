...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        doMethod(before, page, request, view, response, pageClass);
        if (response.getStatus() == HttpServletResponse.SC_FOUND) {
            return;
        }
        doMethod(method, page, request, view, response, pageClass);
        if (response.getStatus() == HttpServletResponse.SC_FOUND) {
            return;
        }
        doMethod(after, page, request, view, response, pageClass);
        if (response.getStatus() == HttpServletResponse.SC_FOUND) {
            return;
        }
    ...
    private void doMethod(Method method, Object page, HttpServletRequest request, Map<String, Object> view, HttpServletResponse response, Class<?> pageClass)
