...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        doMethod(json, before, page, request, view, response, pageClass);
        if (response.getStatus() == HttpServletResponse.SC_FOUND || view.containsKey("redirect")) {
            return;
        }
        doMethod(json, method, page, request, view, response, pageClass);
        if (response.getStatus() == HttpServletResponse.SC_FOUND || view.containsKey("redirect")) {
            return;
        }
        doMethod(json, after, page, request, view, response, pageClass);
        if (response.getStatus() == HttpServletResponse.SC_FOUND || view.containsKey("redirect")) {
            return;
        }
    ...
    private void doMethod(boolean json, Method method, Object page, HttpServletRequest request, Map<String, Object> view, HttpServletResponse response, Class<?> pageClass)
    ...
    private Method getMethod(Class<?> pageClass, String action) {
        Method method = null;
