...
public class Page {
    ...
    protected boolean isNotLoggedIn(HttpSession httpSession) {
        if (isNull(getUser(httpSession))) {
            setMessage(httpSession, "You need to log in to access this page.");
        }
        return isNull(getUser(httpSession));
    }
}
